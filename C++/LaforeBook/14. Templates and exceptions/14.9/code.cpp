//9. Измените программу из предыдущего упражнения таким образом, чтобы в сообщение об ошибке входила информация о значении индекса, приведшего к сбою.
...
class SafeArray
{
	...
	class ErrorIncorrectIndex
	{
		...
		void display(){cout<<"The index "<<IncorrectIndex<<" is incorrect\n";}
	};
...
int main()
{
	...
	for(i=0; i<LIMIT+3; i++)
	{
		...
		catch(SafeArray<int>::ErrorIncorrectIndex NewError){NewError.display();}
	}
	for(i=0; i<91; i++)
		cout<<'-';
	...
	for(i=0; i<LIMIT+3; i++)
	{
		...
		catch(SafeArray<long long>::ErrorIncorrectIndex NewError){NewError.display();}
	}
	for(i=0; i<91; i++)
		cout<<'-';
	...
	for(i=0; i<LIMIT+3; i++)
	{
		...
		catch(SafeArray<double>::ErrorIncorrectIndex NewError){NewError.display();}
	}
