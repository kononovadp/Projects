...
void TestObjList()
{
	...
	<<"--------------------Variant task solution--------------------"<<endl;
	List<CFile> DeletedElements;
	srand(time(NULL));
	int count=rand()%size()+1;
	cout<<"deleted elements (quantity generated random):"<<endl;
	...
	cout<<"list of deleted elements:"<<endl;	
	DeletedElements.print();	
	cout<<"Changed file name of random file (extension not changed!):"<<endl;
	srand(time(NULL));
	n=rand()%size();
	it=begin();
	for(int i=1; i<n-1; i++)
	...
	ln=*it;
	t=ln.data.GetDate();
	cout<<"List after removing of files, created earlier then "<<t<<":"<<endl;
	it=begin();
	while(it!=NULL)
	{
		ln=*it;
