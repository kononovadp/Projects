...
void decimal_str_to_mpq(char *bignum, mpq_t& out)
{
	...
	while (*bignum==' ' || *bignum=='\t' || *bignum=='\n') // Trim spaces
		bignum++;
	while (len>0 && (bignum[len-1]==' ' || bignum[len-1]=='\t' || bignum[len-1]=='\n'))
		bignum[--len] = '\0';
	// if (len == 0) return 2;
	// Handle sign
	int sign = 1;
	size_t pos = 0;
	if (bignum[pos] == '+' || bignum[pos]=='-')
	{
		if (bignum[pos]=='-')
			sign = -1;
		pos++;
...
char* mpq_to_decimal(mpq_t q)
{
	/*mpf_t f;
	mpf_init2(f, (MaxFractPartLength + 10) * 8); // precision in bits
	mpf_set_q(f, q);
	char* result = NULL;
	gmp_asprintf(&result, "%.*Ff", MaxFractPartLength, f);
	int i,ResultLength=strlen(result);
	...
	char *out = (char*)malloc(strlen(int_str) + frac_digits + 5);
	char *p = out;
	if (negative) *p++ = '-';
	strcpy(p, int_str);
	p += strlen(int_str);
...
char* OptimizedBignum::plus(char* bignum1, int Bignum1Length, char* bignum2, int Bignum2Length)
{
	...
	mpq_init(result);
	decimal_str_to_mpq(bignum1, num1);
	decimal_str_to_mpq(bignum2, num2);
	mpq_add(result, num1, num2);
	int PrevMaxFractPartLength=MaxFractPartLength;
	MaxFractPartLength=Bignum1Length+Bignum2Length;
	char* res = mpq_to_decimal(result);
...
static int mpfr_to_mpq_via_string_safe(mpq_t rop, mpfr_t f)
{
	...
	// Determine denominator: ifexp10 >= mlen -> integer * 10^(exp10-mlen)
...
{
	if(mpz_sgn(exp_int) == 0)
	{
		mpq_set_ui(rop, 1, 1);
		return 0;
	}
	if(mpz_sgn(exp_int) < 0)
		return -1; // caller should pass non-negative only
	// result = 1
...
static int try_exact_rational_power(mpq_t rop, const mpq_t base, const mpz_t p, const mpz_t q, int negate_exp)
{
	...
	if(base_neg && (q_ul % 2 == 0))
		return -1; // even root of negative is non-real
	...
	if(num_ok && den_ok)
	{
		if(base_neg && (q_ul % 2 == 1))
			mpz_neg(root_num, root_num);
...
static int approximate_pow_with_retry(mpq_t rop, const mpq_t base, const mpq_t exp)
{
	...
	// If base negative and exponent non-integer -> reject early ifdenominator even
	if(mpq_sgn(base) < 0)
	{
		mpz_t q;// = {0};
	...
	while(prec <= max_prec)
	{
		...
		if(mpfr_sgn(fbase) < 0)
		{
			...
			// For safety, compute pow(|base|, exp) and later reapply sign ifexponent is rational with odd denom.
			mpfr_t fabsbase;
			...
			{
				// integer exponent: ifodd, negative
			...
			else
			{
				if(mpz_fits_ulong_p(qtmp))
				{
					...
					if((q_ui & 1u) != 0)
						// ifnumerator odd -> negative (approx heuristic)
						if(mpz_odd_p(mpq_numref(exp)))
							should_neg = 1;
				}
			}
			mpz_clear(qtmp);
			if(should_neg)
				mpfr_neg(fres, fres, MPFR_RNDN);
		}
		else
		...
		if(mpfr_regular_p(fres))
		{
			...
			if(mpfr_inf_p(fres) || mpfr_nan_p(fres))
			{
				mpfr_clears(fbase, fexp, fres, (mpfr_ptr)0);
		...
		if(ed == 0.0)
			prec = (prec < max_prec/2) ? prec*2 : max_prec;
		else
		{
...
int power(mpq_t rop, const mpq_t base, const mpq_t exp)
{
	...
	if(mpz_cmp_ui(q, 1) == 0)
	{
		if(exp_neg)
		{
			...
			mpq_clear(new_exp);		  // clear old exp ifalready initialized
	...
	// ifnegative base and q even -> non-real
	if(mpq_sgn(base) < 0 && mpz_even_p(q))
	{
		mpz_clears(p, q, NULL);
		mpq_clear(new_exp);
		return -1;
	}
	// try exact rational method ifp,q small
	if(mpz_fits_ulong_p(p) && mpz_fits_ulong_p(q))
	{
		// exp_neg=0;
		unsigned long p_ul = mpz_get_ui(p);
...
{
	// Can't validate ifexp is zero (result should be 1)
	if(mpq_sgn(exp) == 0)
	{
	...
	// Check ifrelative error is within tolerance
...
int validate_power_result_log(const mpq_t result, const mpq_t base, const mpq_t exp, double tolerance)
{
	...
	mpfr_inits2(prec, f_base, f_exp, f_result, f_log_base, f_log_result, f_expected_log, f_diff, NULL);
	// Convert to MPFR
	mpfr_set_q(f_base, base, MPFR_RNDN);
	mpfr_set_q(f_exp, exp, MPFR_RNDN);
	mpfr_set_q(f_result, result, MPFR_RNDN);
	...
	mpfr_clears(f_base, f_exp, f_result, f_log_base, f_log_result, f_expected_log, f_diff, NULL);
...
int validate_power_comprehensive(const mpq_t result, const mpq_t base, const mpq_t exp, double tolerance)
{
	...
	return -1; // All validation methods failed
}
