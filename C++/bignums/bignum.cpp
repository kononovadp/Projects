...
char* bignum::plus(char* bignum1,int Bignum1Length,char* bignum2,int Bignum2Length)
{
	...
	{
		char *result=(char*)malloc(Bignum2Length+1);
		strcpy(result,bignum2);
		return result;
	}
	if(Bignum2Length==1 && bignum2[0]=='0')
	{
		char *result=(char*)malloc(Bignum1Length+1);
	...
	char SumOfDigits,*result=(char*)malloc(MaxBignumLength+MaxFractPartLength+Bignum1Length+Bignum2Length);
...
char* bignum::minus(char* bignum1,int Bignum1Length,char* bignum2,int Bignum2Length)
{
	...
	if(bignum1[0]=='-' && bignum2[0]=='-')
	{
		...
		if(bignum1[0]!='-' && bignum2[0]=='-')
		{
			...
			if(SignMinus)
			{
				...
				PlusResult=(char*)realloc(PlusResult,ResultLength+2);
	...
	if(IsLess(bignum1,Bignum1Length,bignum2,Bignum2Length))
		SwapBignums(bignum1,Bignum1Length,bignum2,Bignum2Length);
	char difference,*result=(char*)malloc(MaxBignumLength+MaxFractPartLength+Bignum1Length+Bignum2Length);
	int Operand1PointPosition=GetPointPosition(bignum1,Bignum1Length);
	int Operand2PointPosition=GetPointPosition(bignum2,Bignum2Length);
	if(Operand1PointPosition==Bignum1Length && Operand2PointPosition==Bignum2Length)
	{
...
char* bignum::multiply(char* bignum1,int Bignum1Length,char* bignum2,int Bignum2Length)
{
	...
	char MultiplicationResult,carry=0,*result=(char*)malloc(MaxBignumLength+MaxFractPartLength+Bignum1Length+Bignum2Length);
	int Operand1PointPosition=GetPointPosition(bignum1,Bignum1Length);
	int Operand2PointPosition=GetPointPosition(bignum2,Bignum2Length);
	int ResultLastIndex=Bignum1Length+Bignum2Length+SignMinus;
	int ResultLength=ResultLastIndex;
	char* multiplication=(char*)malloc(MaxBignumLength+MaxFractPartLength+Bignum1Length+Bignum2Length);
...
{	
	...
	int ResultSize=MaxBignumLength+Bignum1Length+Bignum2Length*10;
	char* bignum1=(char*)malloc(ResultSize);
	char* bignum2=(char*)malloc(ResultSize);
	...
	int i,j,k,l,DividendStartIndex,CountOfZeros;
	int CurRemainderLength,FractPartSize,CountOfRemainders;
	char OperationResult,carry=0,**remainders;
	char* DividendAndRemainder=(char*)malloc(ResultSize);
	char* MultiplicationResult=(char*)malloc(ResultSize);
	DividendAndRemainder[ResultSize-1]='\0';
	MultiplicationResult[ResultSize-1]='\0';
	char** MultiplicationResults=(char**)malloc(sizeof(char*)*11);
	int* MultiplicationResultsStartIndexes=(int*)malloc(11*sizeof(int));
	int* MultiplicationResultsLengths=(int*)malloc(11*sizeof(int));
	char* PrevOperand1,*PrevOperand2,*LengthsOfRemainders;;
	...
	if(Operand1PointPosition!=Bignum1Length || Operand2PointPosition!=Bignum2Length)
	{
		...
		if(Operand1PointPosition!=Bignum1Length)
		{
			...
			PrevOperand1=(char*)malloc(ResultSize);
		...
		if(Operand2PointPosition!=Bignum2Length)
		{
			...
			PrevOperand2=(char*)malloc(ResultSize+1);
	...
	MultiplicationResults[1]=(char*)malloc(ResultSize);
	...
	if(!ResultWasGot)
	{
		DividendAndRemainder[DividendLength++]='0';
		DividendAndRemainder[DividendLength]='\0';
		LengthsOfRemainders=(char*)malloc(MaxBignumLength+1);
		remainders=(char**)calloc(sizeof(char*),MaxBignumLength+1);
		remainders[0]=(char*)malloc(ResultSize);		
		...
		if(MultiplicationResultIndex>0)
		{
			...
			if(!ResultWasGot)
			{
				for(i=0; i<DividendLength && DividendAndRemainder[i]=='0'; i++);
				...
				for(k=i; i<DividendLength; i++)
					if(DividendAndRemainder[i]!='0')
					{
						...
						{
							if(!remainders[CountOfRemainders])
								remainders[CountOfRemainders]=(char*)malloc(ResultSize);
	...
	for(i=1; i<11; i++)
		free(MultiplicationResults[i]);
	free(MultiplicationResults);
	free(DividendAndRemainder);
	free(MultiplicationResult);
	free(MultiplicationResultsStartIndexes);
	free(MultiplicationResultsLengths);
	free(bignum1);
	free(bignum2);
	return result;
}
