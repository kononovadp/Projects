...
void elevator::decide() //принятие решения
{
	...
	if(current_dir!=STOP)
		return;
	int DistanceToHigherRequest=0,DistanceToLowerRequest=0;
	if(requests_above)
		DistanceToHigherRequest=nearest_higher_req-current_floor;	
	if(requests_below)
		DistanceToLowerRequest=current_floor-nearest_lower_req;
	int CountOfUpElevators=0,CountOfDownElevators=0;
	int CountOfUpDirections=0,CountOfDownDirections=0;
	for(j=0; j<NUM_CARS; j++)
	{
		...
		if(odir==UP)
			...
			if(odir==DN)
				CountOfDownElevators++;
	}
	for(j=0; j<NUM_FLOORS; j++)
	{
		CountOfUpDirections+=ptrBuilding->get_floor_req(UP,j);
		CountOfDownDirections+=ptrBuilding->get_floor_req(DN,j);
	}
	if(CountOfUpElevators+CountOfDownElevators>=CountOfDownDirections+CountOfUpDirections)
		return;
	//проверка, есть ли другие лифты, (a) того же направления между нами и ближайшим ЗЭ, или
	...
	for(j=0; j<NUM_CARS; j++) //проверить каждый лифт 
	{
		...
		{ 
			ofloor=ptrBuilding->get_cars_floor(j); //получить его этаж
			odir=ptrBuilding->get_cars_dir(j); //и направление
			...
			if((/*odir==UP || */odir==STOP) && requests_above)
				...
				if((ofloor>current_floor && ofloor<=nearest_higher_req)
					...
					if(nearest_higher_req-ofloor<DistanceToHigherRequest)
					car_between_up=true;
			...
			if((/*odir==DN || */odir==STOP) && requests_below)
				...
				if((ofloor<current_floor && ofloor>=nearest_lower_req)
					...
					|| (ofloor==current_floor && j<car_number))
					if(ofloor-nearest_lower_req<DistanceToLowerRequest)
					car_between_dn=true;
	...
	//если идем вниз или остановились, и снизу есть ЗЭ, и нет лифтов, идущих вниз, между нами и ЗЭ или под ЗЭ, идущих вверх и ближе нас к ЗЭ
...
void elevator::get_destinations() //остановка,получение пунктов назначения
{
	...
	{																	 
		...
		dest_floor=GetIntFromKeyboard(0,NUM_FLOORS);
