	...
	{		
		...
		switch(choice)
		{
			case 0:
				...
				strcat(MyLangCode,"<< ");
				for(i=0; i<CountOfWritelnVariables; i++)
				{
			...
			case 2:
				...
				AddRandomArithmeticExpression(DelphiCode,MyLangCode,variables,CountsOfVariables,operations,GetRandomDigit(3,6));
				strcat(DelphiCode,";\n");
				strcat(MyLangCode,";\n");
			break;
		}
...
int main(void)
{
	// char* code=GetDelphiAndMyLangCode();
	int i,j;
	char ch,*code=(char*)calloc(65536,sizeof(char));
	ifstream MyLangFile("test.mylang");
	MyLangFile.read(&ch,sizeof(char));
	for(i=0; !MyLangFile.eof(); i++)
	{
		code[i]=ch;
		MyLangFile.read(&ch,sizeof(char));
	}
	MyLangFile.close();
	struct token*** tokens=LexicalAnalyzer(code);
