...
//*****************************************************************
CBinTree* findstationinf(string st,CBinTree *tree)
{
	if(tree!=NULL)
	if(tree->data->GetStationName()==st)
		tree->data->Display();
	else
	if(tree!=NULL)
	{
		if(st<tree->data->GetStationName())
			findstationinf(st,tree->l);
		else
			if(st>tree->data->GetStationName());
				findstationinf(st,tree->r);
	}
	//return tree;
}
void showstationinf(string st,CBinTree *tree)
{
	cout<<endl<<"Information about trains bound for "<<st;
	//CBinTree *t=
	findstationinf(st,tree);
	//t->data->Display();
}
/*int findstationinf(string st,CBinTree *tree,int n)
{
	if(tree==NULL)
		return NULL;
	else
	...
	if(tree!=NULL)
		if(st<tree->data->GetStationName())
			return findstationinf(st,tree->l,n);
			else
				return findstationinf(st,tree->r,n);
}
void showstationinf(string station,CBinTree *tree)
{
	cout<<endl<<"Information about trains bound for "<<station;
	int t=findstationinf(station,tree,0);	
	if(t==0)
		cout<<" not found";
...
int main()
{
	...
	tree->showtree(tree);
	cout<<endl<<"-----------------------------------";*/
	//tree->showtraininf(2,tree);
	tree->showstationinf("Nikolaev",tree);
	cin.get();
}