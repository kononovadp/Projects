/*Модифицируйте программу rain.c из листинга 10.7, чтобы она выполняла вычисления с использованием указателей вместо индексов. (Вам по-прежнему придется объявлять и инициализировать массив.)*/
...
int main(void)
{
    ...
    for (i=0, total=0; i<MONTHS*YEARS; i+=MONTHS)
    {   // для каждого года суммировать количество осадков за каждый месяц
        for (ptr=*rain+i, subtot = 0; ptr<*rain+MONTHS+i; ptr++)
            subtot += *ptr;
        printf("%5d %15.1f\n", 2010+i/MONTHS, subtot);
        total += subtot; // общая сумма для всех лет
    }
	...
	for(i=0; i<MONTHS; i++)
	{
		for(subtot=0,ptr=*rain+i; ptr<*rain+i+1*MONTHS*YEARS; ptr+=MONTHS)
			subtot+=*ptr;
		printf("%4.1f ", subtot/YEARS);
	}
    return 0;
