...
//BouldersCount - кол-во валунов, towers - массив результатов, BusyBoulders - помеченные использованные валуны для всех башен, rescount - кол-во башен каждого размера, boulders - все валуны, MinHigh - минимальная высота валуна, RequiredHigh - высота, которую пытаемся получить.
void build(int BouldersCount,int ***towers,char ***BusyBoulders,int *TowersCount,int ***boulders,int MinHigh,int RequiredHigh)
{
		...
		for(j=0; j<3; j++)
		{
			...
			//если такая конструкция есть, шагаем по всем вариантам одной и той же высоты: смотрим, не входит ли в ее состав текущий камень и можно ли ее установить на текущий валун
				for(z=0; z<TowersCount[MinusResult]; z++)
					if((BusyBoulders[MinusResult][z][i]==0) && ((boulders[i][j][0]>=towers[MinusResult][z][0] && boulders[i][j][1]>=towers[MinusResult][z][1]) || (boulders[i][j][0]>=towers[MinusResult][z][1] && boulders[i][j][1]>=towers[MinusResult][z][0])))
					{
						...
						towers[RequiredHigh][TowersCount[RequiredHigh]]=(int*)calloc(3,sizeof(int));
						for(x=0; x<3; x++)
							towers[RequiredHigh][TowersCount[RequiredHigh]][x]=boulders[i][j][x];
...
int GetMaxHigh(int BouldersCount,int ***boulders)
{
	...
	for(i=0,MaxHigh=boulders[0][0][2]; i<BouldersCount; i++)
		for(j=0; j<3; j++)
		{
			...
			towers[boulders[i][j][2]][TowersCount[boulders[i][j][2]]]=(int*)calloc(3,sizeof(int));
			for(k=0; k<3; k++)
				towers[boulders[i][j][2]][TowersCount[boulders[i][j][2]]][k]=boulders[i][j][k];
	...
	for(i=MinHigh; i<TotalHigh; i++)
		if(towers[i]==NULL)
			build(BouldersCount,towers,BusyBoulders,TowersCount,boulders,MinHigh,i);
	i=TotalHigh-1;
	while(towers[i]==NULL)
		i--;
	if(i>MaxHigh)
