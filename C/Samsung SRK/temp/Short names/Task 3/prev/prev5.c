...
int **CheckTower(int tc,int **mem,int freepos,int *add,FILE *f)
{
	int i,j,**t,res=1;
...
{
	int i,j,k,l,x,y,rg,top,pos,mr,**res;
	if(pr[rn]!=NULL)
		return 0;
	for(i=0; i<tc; i++)
		for(j=0; j<3; j++,invert(tow[i]))
		{
			...
			{
				res=CheckTower(tc,pr[mr],i,tow[i],f);
				if(res!=NULL)
				{
					pr[rn]=res;
					return 1;
...
{
	int i,j,k,t,l,sn,res,min,max,***pr,sum=0;
	...
	pr=(int***)calloc(sum+1,sizeof(int**));
	for(i=min,sum++; i<sum; i++)
	{
