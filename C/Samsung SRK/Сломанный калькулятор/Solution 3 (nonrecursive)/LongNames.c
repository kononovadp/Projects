...
int calc(char *operations,int OperationsCount,int MaximumPressesCount,unsigned long long RequiredNumber,long long MaxForDivisionAndSubtraction,int *cache,char PlusMult,char MinusDiv,unsigned long long **operands,char *OperationsForResult)
{
	unsigned long long i,OperationResult,operand1,operand2;
	int j,PressesCount,MininmalPressesCount=MaximumPressesCount;
		...
		for(i=RequiredNumber-1; i>0; i--)
			if(cache[i]>0) //если число уже было получено
				for(j=0; j<OperationsCount; j++)
				{
					OperationResult=0;
					if(operations[j]=='+')
						OperationResult=RequiredNumber-i;
					else
						if(operations[j]=='*' && RequiredNumber%i==0)
							OperationResult=RequiredNumber/i;
					//если результат операции больше 0 и он был уже ранее получен
					if(OperationResult>0 && cache[OperationResult]>0)
					{
						PressesCount=cache[i]+cache[OperationResult]+1;
						if(PressesCount<MininmalPressesCount)
						{
							MininmalPressesCount=PressesCount;
							//сохранение "пути" к результату
							operand1=i;
							operand2=OperationResult;
		...
		for(i=RequiredNumber+1; i<MaxForDivisionAndSubtraction; i++)
		{
			if(cache[i]>0) //если число уже было получено
				for(j=0; j<OperationsCount; j++)
				{
					OperationResult=0;
					if(operations[j]=='-')
						OperationResult=i-RequiredNumber;
					else
					...
					if(OperationResult>0 && cache[OperationResult]>0)
					{
						...
						if(PressesCount<MininmalPressesCount)
						{
							...
							if(OperationsForResult[OperationResult]=='*')
								OperationsForResult[OperationResult]='/';
							else
								if(OperationsForResult[OperationResult]=='+')
									OperationsForResult[OperationResult]='-';
						}
...
{
	int *NumbersWithDigits,j,*cache,result,OperationsCountForResult=0;
	...
	else
	{
		...
		for(i=1; i<=RequiredNumber; i++)
			if(cache[i]==0)
				cache[i]=calc(operations,OperationsCount,MaximumPressesCount,i,MaxForDivisionAndSubtraction,cache,PlusMult,MinusDiv,operands,OperationsForResult);
		result=cache[RequiredNumber];
