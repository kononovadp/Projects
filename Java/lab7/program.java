...
class program
{
	...
	interface CountOfPrimeNumbers
	{
		int GetCountOfSuperPrimeNumbers(int[] numbers, int count);
	}
	public static void main(String args [])
	{
		...
		CountOfPrimeNumbers interf=(arr1,n)->
		{
			int CountOfPrimeNumbers=0;
			for(int i=0; i<n; i++)
			{
				if(IsNumberPrime(arr1[i])==1)
				{
					CountOfPrimeNumbers++;
					System.out.print(CountOfPrimeNumbers+". The number "+arr1[i]+" is prime\n");
				}
			}
			return CountOfPrimeNumbers;
		};
		System.out.println("\n--------------------lambda--------------------\n");
		int CountOfPrimeNumbers=interf.GetCountOfSuperPrimeNumbers(arr,ArraySize);
		System.out.print("----------------------------------------------------------\nCount of super prime numbers: "+CountOfPrimeNumbers);
    }
}