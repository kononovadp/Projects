...
uses
  ...
  Buttons, ExtCtrls, ComCtrls, ValEdit, Spin,crt,dos,Windows,bignums;
...
type
  ...
  TForm1 = class(TForm)
    ...
    bracket1: TButton;
    bracket2: TButton;
    MinCountOfOperations_SpinEdit: TSpinEdit;
    GenerateAndCalculate_Button: TButton;
    MaxCountOfDigits_SpinEdit: TSpinEdit;
    CountOfExpressions_SpinEdit: TSpinEdit;
    MinCountOfOperations_Label: TLabel;
    ...
    ae: TEdit;
    calctime: TCheckBox;
    CountOfExpressions_Label: TLabel;
    MinCountOfDigits_Label: TLabel;
    MaxCountOfDigits_Label: TLabel;
    MaxCountOfOperations_Label1: TLabel;
    MaxCountOfOperations_SpinEdit: TSpinEdit;
    rope_type: TComboBox;
    Label17: TLabel;
    rope_cond: TComboBox;
    rope_effort: TEdit;
    ...
    Label15: TLabel;
    Label16: TLabel;
    MinCountOfDigits_SpinEdit: TSpinEdit;
    ...
    procedure charinput(ch:string);
    procedure aeClick(Sender: TObject);
    procedure GenerateAndCalculate_ButtonClick(Sender: TObject);
    procedure NumberButton(Sender: TObject);
    ...
    procedure deactivate_screenkeyboard(Sender: TObject);
...
s1,s2{початкові рядки},od{залишок від ділення},
se1,se2{вирівняні рядки}:string;
sys{система числення},sys1,sys2,
prec{точність ділення}:byte;
z1,z2{позиції ком}:integer;
{арифметичні операції}op_plusb,op_minusb,
{поля вводу}b_ae,b_quantity,b_thickness,
b_kgm,b_rope_effort,b_divprec,converter:boolean;
implementation
{$R *.lfm}
...
//вирівнювання чисел без коми
procedure eqsize(var s1,s2:string);
var i:byte;
begin
...
se1:=s1; se2:=s2; end;
//видалення нуля на початку рядка
...
//перевірка відповідності числа вказаній системі числення
function check(s:string):boolean;
var i:byte; myset:set of char; t:string;
begin myset:=[];
...
myset:=myset+[chr(i)];
if sys>10 then
for i:=65 to 65+sys-9 do
myset:=myset+[chr(i)]; i:=1;
for i:=1 to length(s) do
if not(s[i] in myset) then begin check:=false; exit; end
else check:=true; end;
//обробка знаку при множенні ті діленні
...
//додавання чисел без знаку
function op_plus(s1,s2:string):string;
var e:string; c,d,i,r,p,q,cp: byte;
begin r:=0; e:=''; cp:=0;
if pos(comma,s1)>0 then z1:=length(s1)-pos(comma,s1) else z1:=0;
if pos(comma,s2)>0 then z2:=length(s2)-pos(comma,s2) else z2:=0;
...
e:=zerodelb(e);
if op_plusb=false then begin
if cp>0 then insert(comma,e,length(e)-cp+1);
if pos(comma,e)=1 then e:='0'+e;
if pos(comma,e)=length(e) then delete(e,pos(comma,e),1);
if pos(comma,e)=0 then e:=zerodelb(e) else begin
e:=zerodele(e); e:=zerodelb(e); end;
if pos(comma,e)=length(e) then delete(e,length(e),1); end;
op_plus:=e; end;
//віднімання чисел без знаку
...
if s[1]='0' then  while s[1]='0' do delete(s,1,1);
i:=1;
if pos(comma,s)=1 then s:='0'+s;
if pos(comma,s)=length(s) then delete(s,pos(comma,s),1);
if ms=true then s:='-'+s; end;
op_minus:=s; end;
//додавання зі знаком
function plus(s1,s2:string):string;
begin
if (check(s1)=false) or (check(s2)=false) then begin
plus:='Enter correct operands!'; exit; end;
if (s1[1]='-') and (s2[1]='-') then begin
...
//віднімання зі знаком
function minus(s1,s2:string):string;
begin
if (check(s1)=false) or (check(s2)=false) then begin
minus:='Enter correct operands!'; exit; end;
minus:='';
if (s1[1]='-') and (s2[1]='-') then begin
delete(s1,1,1); delete(s2,1,1);
...
var e,p,p2:string;
    i,j,l1,i1,j1,a,b,r,c,d,n,n1:longint;
    ms:boolean;
begin
if (check(s1)=false) or (check(s2)=false) then begin
mult:='Enter correct operands!'; exit; end;
...
begin
if (check(s1)=false) or (check(s2)=false) then begin
divide:='Enter correct operands!'; exit; end;
...
//перетворення з десяткової системи числення в будь-яку іншу
...
if (r1[1]='0') and (pos(comma,r1)>2) then
while r1[1]='0' do delete(r1,1,1);
if r1[1]=comma then r1:='0'+r1;
...
if (t1[1]='0') and (pos(comma,t1)>2) then
while t1[1]='0' do delete(t1,1,1);
if t1[1]=comma then t1:='0'+t1;
numberconvert:=t1; sys1:=nst; end;
//обробка виразів без дужок
function strprocwb({var }s:string):string;
var s1,s2:string; c:char; pr,i:byte;
myset:set of char; b:boolean;
...
//обробка виразів з дужками
function strproc(s:string):string;
var r,t1,t2:string; p,i:byte;
begin
...
//================================кнопки вводу==================================
procedure TForm1.charinput(ch:string);
begin
...
begin if b_ae=true then begin ae.text:=ae.text+'-';
ae.SetFocus; ae.SelStart:=length(ae.text); end; end;
...
procedure TForm1.GenerateAndCalculate_ButtonClick(Sender: TObject);
var
	output: PChar;
begin
	MinCountOfDigits:=MinCountOfDigits_SpinEdit.Value;
	MaxCountOfDigits:=MaxCountOfDigits_SpinEdit.Value;
	CountOfExpressions:=CountOfExpressions_SpinEdit.Value;
	MinCountOfOperations:=MinCountOfOperations_SpinEdit.Value;
	MaxCountOfOperations:=MaxCountOfOperations_SpinEdit.Value;
	UpdateLimits;
	output:=TestExpressions();
	memo.text:='';
	memo.text:=output;
end;
...
var s:string; t : Longword; tr:real;
begin converter:=false;
if ae.text<>'' then begin
...
if calctime.state=cbunChecked then memo.text:=memo.text+ ';'+#13 else
memo.text:=memo.text+ ' Час обчислення: '+
FormatFloat('0.####',tr)+' секунд '+#13+#13;
end else showmessage('Операція відсутня!');
ae.SetFocus; end;
...
begin
if MetallType.Text='Сортовий' then
...
begin
  if unit_w_l.text='Вага' then begin
  calc_w_l.Caption:='Обчислити вагу троса';
  label11.Caption:='Вага троса (кг)';
  m_unit1.caption:='Кількість метрів'; end else begin
  calc_w_l.Caption:='Обчислити довжину троса';
  m_unit1.caption:='Кількість кілограм';
  label11.Caption:='Довжина троса (м)'; end;
end;
//======================Розривні зусилля та діаметр каната======================
...
begin
  ...
  if rope_type.text='Трьохпрядний конопляний' then
  assignfile(f, 'rope-diameter_hemp.txt') else
  if rope_type.text='Сталевий (6х36 - 216 дротів)' then
  ...
  rope_d.text:='Дуже велике зусилля!'; closefile(f);
end;
...
//==============================================================================
procedure TForm1.deactivate_screenkeyboard(Sender: TObject);
begin
  b_ae:=false;  b_quantity:=false;
  b_thickness:=false; b_kgm:=false;
  b_rope_effort:=false; b_divprec:=false;
