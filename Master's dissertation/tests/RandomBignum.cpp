...
bool IsLess(char* bignum1,int Bignum1Length,char* bignum2,int Bignum2Length)
{
	...
	if(bignum1[0]=='-' && bignum2[0]=='-')
	{
		...
		if(Operand1FractPartSize<Operand2FractPartSize)
		{
			...
			if(Operand1FractPartSize>Operand2FractPartSize)
			{
				for(i=Operand2PointPos+1; bignum1[i]==bignum2[i]; i++);
				for(; i<Bignum2Length; i++)
				{
					if(bignum1[i]<bignum2[i])
						return false;
...
{
	int i,MinLength=strlen(min),MaxLength=strlen(max);
...
void SetRandomFloatBignum(char* number,int& length,char* min,char* max,int MinPointPos,int MaxPointPos)
{
	...
	char* MinFractPart=(char*)calloc(MaxBignumLength+MaxFractPartLength+3,sizeof(char));
	char* MaxFractPart=(char*)calloc(MaxBignumLength+MaxFractPartLength+3,sizeof(char));
	char* RandomFractPart=(char*)calloc(MaxBignumLength+MaxFractPartLength+3,sizeof(char));
...
char* GetRandomBignum(char* min,char* max)
{
	...
	char* ZeroOrOne=(char*)calloc(MaxBignumLength+MaxFractPartLength+3,sizeof(char));
	char* number=(char*)calloc(MaxBignumLength+MaxFractPartLength+3,sizeof(char));
	ZeroOrOne[1]='\0';
	// ZeroOrOne->length=1;
	if(min[0]=='-' && max[0]=='-')
	{
