...
uses
  ...
  Buttons, ExtCtrls, Grids, MaskEdit,crt,dos;
...
op_plusb,op_minusb,
b_ae,b_quantity,b_thickness:boolean;
implementation
{$R *.lfm}
...
begin
if b_ae=true then begin ae.text:=ae.text+'0';
ae.SetFocus; ae.SelStart:=length(ae.text); end else
if b_quantity=true then begin quantity.text:=quantity.text+'0';
quantity.SetFocus; quantity.SelStart:=length(quantity.text); end
else begin thickness.text:=thickness.text+'0';
thickness.SetFocus; thickness.SelStart:=length(thickness.text); end; end;
procedure TForm1.b1Click(Sender: TObject);
begin
if b_ae=true then begin ae.text:=ae.text+'1';
ae.SetFocus; ae.SelStart:=length(ae.text); end else
if b_quantity=true then begin quantity.text:=quantity.text+'1';
quantity.SetFocus; quantity.SelStart:=length(quantity.text); end
else begin thickness.text:=thickness.text+'1';
thickness.SetFocus; thickness.SelStart:=length(thickness.text); end; end;
procedure TForm1.b2Click(Sender: TObject);
begin
if b_ae=true then begin ae.text:=ae.text+'2';
ae.SetFocus; ae.SelStart:=length(ae.text); end else
if b_quantity=true then begin quantity.text:=quantity.text+'2';
quantity.SetFocus; quantity.SelStart:=length(quantity.text); end
else begin thickness.text:=thickness.text+'2';
thickness.SetFocus; thickness.SelStart:=length(thickness.text); end; end;
procedure TForm1.b3Click(Sender: TObject);
begin
if b_ae=true then begin ae.text:=ae.text+'3';
ae.SetFocus; ae.SelStart:=length(ae.text); end else
if b_quantity=true then begin quantity.text:=quantity.text+'3';
quantity.SetFocus; quantity.SelStart:=length(quantity.text); end
else begin thickness.text:=thickness.text+'3';
thickness.SetFocus; thickness.SelStart:=length(thickness.text); end; end;
procedure TForm1.b4Click(Sender: TObject);
begin
if b_ae=true then begin ae.text:=ae.text+'4';
ae.SetFocus; ae.SelStart:=length(ae.text); end else
if b_quantity=true then begin quantity.text:=quantity.text+'4';
quantity.SetFocus; quantity.SelStart:=length(quantity.text); end
else begin thickness.text:=thickness.text+'4';
thickness.SetFocus; thickness.SelStart:=length(thickness.text); end; end;
procedure TForm1.b5Click(Sender: TObject);
begin
if b_ae=true then begin ae.text:=ae.text+'5';
ae.SetFocus; ae.SelStart:=length(ae.text); end else
if b_quantity=true then begin quantity.text:=quantity.text+'5';
quantity.SetFocus; quantity.SelStart:=length(quantity.text); end
else begin thickness.text:=thickness.text+'5';
thickness.SetFocus; thickness.SelStart:=length(thickness.text); end; end;
procedure TForm1.b6Click(Sender: TObject);
begin
if b_ae=true then begin ae.text:=ae.text+'6';
ae.SetFocus; ae.SelStart:=length(ae.text); end else
if b_quantity=true then begin quantity.text:=quantity.text+'6';
quantity.SetFocus; quantity.SelStart:=length(quantity.text); end
else begin thickness.text:=thickness.text+'6';
thickness.SetFocus; thickness.SelStart:=length(thickness.text); end; end;
procedure TForm1.b7Click(Sender: TObject);
begin
if b_ae=true then begin ae.text:=ae.text+'7';
ae.SetFocus; ae.SelStart:=length(ae.text); end else
if b_quantity=true then begin quantity.text:=quantity.text+'7';
quantity.SetFocus; quantity.SelStart:=length(quantity.text); end
else begin thickness.text:=thickness.text+'7';
thickness.SetFocus; thickness.SelStart:=length(thickness.text); end; end;
procedure TForm1.b8Click(Sender: TObject);
begin
if b_ae=true then begin ae.text:=ae.text+'8';
ae.SetFocus; ae.SelStart:=length(ae.text); end else
if b_quantity=true then begin quantity.text:=quantity.text+'8';
quantity.SetFocus; quantity.SelStart:=length(quantity.text); end
else begin thickness.text:=thickness.text+'8';
thickness.SetFocus; thickness.SelStart:=length(thickness.text); end; end;
procedure TForm1.b9Click(Sender: TObject);
begin
if b_ae=true then begin ae.text:=ae.text+'9';
ae.SetFocus; ae.SelStart:=length(ae.text); end else
if b_quantity=true then begin quantity.text:=quantity.text+'9';
quantity.SetFocus; quantity.SelStart:=length(quantity.text); end
else begin thickness.text:=thickness.text+'9';
thickness.SetFocus; thickness.SelStart:=length(thickness.text); end; end;
...
quantity.SetFocus; quantity.SelStart:=length(quantity.text); end
else begin thickness.text:=thickness.text+ch;
thickness.SetFocus; thickness.SelStart:=length(thickness.text); end; end;
...
begin
if b_ae=true then ae.text:=ae.text+'-' else
if b_quantity=true then quantity.text:=quantity.text+'-'
else thickness.text:=thickness.text+'-'; end;
...
if (check(quantity.text)='') and (check(thickness.text)='') then
s:=s+quantity.text+'*'+thickness.text+'*'+'7.85' end else
if check(quantity.text)='' then
s:=s+quantity.text+'*'+unit_weight.text;
if s<>'' then begin
ae.text:=s;
resClick(Sender);
res_weight.text:=e3.text; end;
end;
...
end else showmessage('Отсутствует операция!'); end;
//--------------------------------------------------------------
...
if calctime.state=cbunChecked then memo.text:=memo.text+ ';'+#13 else
memo.text:=memo.text+ ' Время расчета: '+
FormatFloat('0.####',tr)+' секунд '+#13+#13; end;
...
begin buttonenable(10); aeClick(Sender);
end;
