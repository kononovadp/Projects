...
begin
   ...
   for i:= 1 to 4 do     //i - номер шага
     begin
     k:=i;
     for j:=i+1 to 5 do if a[j,j,p]<a[k,k,p] then k:=j;//к - указатель на самый большой элемент
     if k>i then //меняем элементы

         begin
         m:=a[k,k,p];//откладываем k-й диагональный элемент
         a[k,k,p]:=a[i,i,p]; //в k-й столбец перемещаем элемент из i-го
         a[i,i,p]:=m;//в i-й столбец записываем то что было отложено
         end;
     end;
     end;
