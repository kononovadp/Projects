...
var jqMath = function() {
	...
	function M(x, y, z) /* any number of arguments */ {
		var t = typeof x;
		if (t == 'string' || t == 'number')	return M.sToMathE(String(x), y, z);
	...
	(function() {
		if (self.location) {
			...
			else if ($.browser.safari && /^5[3-4]\d\./.test($.browser.version)
			|| $.browser.opera)
				M.MathML = false;
		}
	})();
	...
	M.newMe = function(tagName, argsP /* for appendMeArgs() */, docP) {
		...
		if (M.MathML)	return newMeNS(tagName, argsP, docP);
	...
	function braceScan() {
		...
		tokP[1] == '}' && ! tokP[0] || F.err(err_braceScan_);
	...
	M.sToMathE = function(s, blockQ, docP) /* converts the formula 's' to an XML 'math' or HTML
			'fmath' element */ {
		typeof s == 'string' && M.infix_[''] && M.infix_[','] || F.err(err_sToMathE_1_);
		if (! docP)	docP = document;
		...
		else if (M.re_.lastIndex < s.length)	F.err(err_sToMathE_2_);
		var res = mxP_tokP[0];
	...
	M.parseMath = function(nod) {
		var aaaa = nod.nodeType; var bbbb = nod.tagName;
