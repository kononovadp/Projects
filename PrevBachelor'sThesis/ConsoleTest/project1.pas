...
H1,M1,Sec1,S1001,H2,M2,Sec2,S1002:Word;
...
//if (s[1]='0') and (pos(comma,s)>0) then exit;
{while s[1]='0' do delete(s1,1,1);
if s[1]=comma then s:='0'+s;
if length(s)=0 then s:='0';}
zerodelb:=s; end;
...
sys1:=8; sys2:=16; s1:= '0,1726354'; prec:=20; tprec:=prec;
op_plusb:=false;  op_minusb:=false; comma:=','; ls1:=length(s1);
writeln('Number  ',s1,'  in system ',sys1,' = ');
gettime(H1,M1,Sec1,S1001);
writeln(numberconvert(s1),'  in system ',sys2,' with precision ',tprec);
gettime(H2,M2,Sec2,S1002);
writeln('TIME: ',inttostr(abs(h2-h1)),' hours ',inttostr(abs(m2-m1)),
' minutes ',inttostr(abs(sec2-sec1)),' seconds Number length: ',ls1);
writeln('=======================================================================');

sys1:=12; sys2:=10; s1:= '1B2A3647589'; prec:=20; tprec:=prec;
op_plusb:=false;  op_minusb:=false; comma:=','; ls1:=length(s1);
writeln('Number  ',s1,'  in system ',sys1,' = ');
gettime(H1,M1,Sec1,S1001);
writeln(numberconvert(s1),'  in system ',sys2,' with precision ',tprec);
...
sys1:=17; sys2:=15; s1:= 'G1F2E3,D4C5B67A89'; prec:=20; tprec:=prec;
op_plusb:=false;  op_minusb:=false; comma:=','; ls1:=length(s1);
writeln('Number  ',s1,'  in system ',sys1,' = ');
gettime(H1,M1,Sec1,S1001);
writeln(numberconvert(s1),'  in system ',sys2,' with precision ',tprec);
...
sys1:=22; sys2:=36; s1:= '0,L2K3J4I5H6G7A8B9CFDE'; prec:=20; tprec:=prec;
op_plusb:=false;  op_minusb:=false; comma:=','; ls1:=length(s1);
writeln('Number  ',s1,'  in system ',sys1,' = ');
gettime(H1,M1,Sec1,S1001);
writeln(numberconvert(s1),'  in system ',sys2,' with precision ',tprec);
...
sys1:=32; sys2:=27; s1:= 'FEDCBAPQRB20,17BHNUV'; prec:=20; tprec:=prec;
op_plusb:=false;  op_minusb:=false; comma:=','; ls1:=length(s1);
writeln('Number  ',s1,'  in system ',sys1,' = ');
gettime(H1,M1,Sec1,S1001);
writeln(numberconvert(s1),'  in system ',sys2,' with precision ',tprec);
...
sys1:=10; sys2:=2; s1:= '908,63'; prec:=35; tprec:=prec;
op_plusb:=false;  op_minusb:=false; comma:=','; ls1:=length(s1);
writeln('Number  ',s1,'  in system ',sys1,' = ');
gettime(H1,M1,Sec1,S1001);
writeln(numberconvert(s1),'  in system ',sys2,' with precision ',tprec);
...
//writeln('DONE!');
converter:=false;
sys:=9; prec:=25; op_plusb:=false;  op_minusb:=false; comma:=',';
...
gettime(H2,M2,Sec2,S1002);
writeln('TIME: ',inttostr(abs(h2-h1)),' hours ',inttostr(abs(m2-m1)),
' minutes ',inttostr(abs(sec2-sec1)),' seconds Operand 1 length: ',ls1,
' Operand 2 length: ',ls2);

sys:=11; prec:=25; op_plusb:=false;  op_minusb:=false; comma:=',';
s1:='-0,0002'; s2:='3,2';
ls1:=length(s1); ls2:=length(s2);
gettime(H1,M1,Sec1,S1001);
writeln('Number system: ',sys, ' Divide precision: ',prec);
...
sys:=17; prec:=25; op_plusb:=false;  op_minusb:=false; comma:=',';
...
sys:=20; prec:=25; op_plusb:=false;  op_minusb:=false; comma:=',';
...
sys:=22; prec:=25; op_plusb:=false;  op_minusb:=false; comma:=',';
s1:='12345167'; s2:='5678';
ls1:=length(s1); ls2:=length(s2);
gettime(H1,M1,Sec1,S1001);
writeln('Number system: ',sys, ' Divide precision: ',prec);
...
sys:=26; prec:=25; op_plusb:=false;  op_minusb:=false; comma:=',';
s1:='35,1'; s2:='1,8';
ls1:=length(s1); ls2:=length(s2);
gettime(H1,M1,Sec1,S1001);
writeln('Number system: ',sys, ' Divide precision: ',prec);
...
sys:=32; prec:=25; op_plusb:=false;  op_minusb:=false; comma:=',';
s1:='3,6'; s2:='0,027';
ls1:=length(s1); ls2:=length(s2);
gettime(H1,M1,Sec1,S1001);
writeln('Number system: ',sys, ' Divide precision: ',prec);
...
sys:=33; prec:=25; op_plusb:=false;  op_minusb:=false; comma:=',';
s1:='ABCDEF9876543210,FEDCBA1'; s2:='10F1E2D3C4B5A60798,89706A5B4C3D2E1F';
ls1:=length(s1); ls2:=length(s2);
gettime(H1,M1,Sec1,S1001);
writeln('Number system: ',sys, ' Divide precision: ',prec);
writeln('Number1: ',s1,' Number2: ',s2);
writeln('+ = ',plus(s1,s2));
writeln('- = ',minus(s1,s2));
writeln('* = ',mult(s1,s2));
writeln('/ = ',divide(s1,s2));
gettime(H2,M2,Sec2,S1002);
writeln('TIME: ',inttostr(abs(h2-h1)),' hours ',inttostr(abs(m2-m1)),
' minutes ',inttostr(abs(sec2-sec1)),' seconds Operand 1 length: ',ls1,
' Operand 2 length: ',ls2);

sys:=36; prec:=30; op_plusb:=false;  op_minusb:=false; comma:=',';
s1:='T01U2VVW34QR5S67NO89P,EKFLDJICBHGA'; s2:='W0V1U2T3S4R5Q6P7O8N9,MALBKCJDIEHFG';
ls1:=length(s1); ls2:=length(s2);
gettime(H1,M1,Sec1,S1001);
writeln('Number system: ',sys, ' Divide precision: ',prec);
writeln('N1: ',s1,' N2: ',s2);
...
{sys:=36; prec:=50; op_plusb:=false;  op_minusb:=false; comma:=',';
s1:='MALBKCJDIEHFGT01UV23WX45YZSRF,QP98ON76MGHBACIJDEKLF';
s2:='W0V1U2T3S4R5Q6P7O8N9,T01U2VVW34QR5S67NO89PMALBKCJDIEHFG';
ls1:=length(s1); ls2:=length(s2);
gettime(H1,M1,Sec1,S1001);
writeln('Number system: ',sys, ' Divide precision: ',prec);
writeln('N1: ',s1); writeln('N2: ',s2);
writeln('+ = ',plus(s1,s2));
writeln('- = ',minus(s1,s2));
writeln('/ = ',divide(s1,s2));
writeln('* = ',mult(s1,s2));
gettime(H2,M2,Sec2,S1002);
writeln('TIME: ',inttostr(h2-h1),' hours ',inttostr(m2-m1),
' minutes ',inttostr(sec2-sec1),' seconds Operand 1 length: ',ls1,
' Operand 2 length: ',ls2);}
writeln('Done!'); readln;
end.
