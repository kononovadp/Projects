...
class GameState:
  ...
  def generateSuccessor( self, agentIndex, action):
    ...
    # Time passes
    if agentIndex == 0: # Penalty for waiting around
      state.data.scoreChange = state.data.scoreChange - TIME_PENALTY - args['penalty']
...
class PacmanRules:
  ...
  def consume( position, state ):
    ...
    if state.data.food[x][y]:
      state.data.scoreChange += 10 + args['reward']
...
def readCommand( argv ):
  ...
  parser.add_option('--reward', dest='reward', type='int',
                    help=default('Reward which can be added to the default value'), default=0)					
  parser.add_option('--penalty', dest='penalty', type='int',
                    help=default('Penalty which can be added to the default value'), default=0)
  ...
  args['reward'] = options.reward
  args['penalty'] = options.penalty
...
def runGames( layout, pacman, ghosts, display, numGames, record, numTraining = 0, catchExceptions=False, timeout=30, reward=0, penalty=0 ):
  import __main__
