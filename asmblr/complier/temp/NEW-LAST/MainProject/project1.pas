...
i:=0; readln(g,s);
readln(f,ms); {if (ms='') or (ms[1]=';') then begin
writeln(f2,ms); readln(f,ms); end; }
while s[i]<>'|' do begin a:=a+s[i]; inc(i); end;
delete(s,1,length(a)); delete(a,1,1); i:=0;
while s[i]<>'/' do begin b:=b+s[i]; inc(i); end;
...
pr2:='67& ';
while not eof(g) do begin
l2:readln(g,s); readln(f,ms); {if (ms='') or (ms[1]=';') then begin
writeln(f2,ms); readln(f,ms); end;}
i:=0; bol:=false;
if pos('*',s)<>0 then
...
while s[i]<>'/' do begin b:=b+s[i]; inc(i); end;
delete(s,1,length(b)); delete(b,1,1); i:=0;
if (b='db') or (b='dw') or (b='dd') or (a='data')
