...
cop:=cop+dt+' R '+cc;
if regn='dword' then insert(pr,cop,1);//если переменная 32-разрядная, то добавляется префикс
//на этом этапе полный код команды сформирован (проблем нет, результат совпадает)
{Теперь нужно сформировать адрес команды, то есть ее смещение, относительно предыдущей команды.
Для этого нужно определить, сколько байт занимает код команды (записан в строку cop).
В нашем случае количество байт - это количество пробелов в строке, но не все так просто.
Количество байт возвращает функция dollar (так называется само смещение в ассемблере).
Если длина константы (то есть длина строки cc) 2 символа,
то ничего прибавлять не нужно. Если 4 символа - то прибавить нужно единицу.
Если 8 символов то прибавить нужно тройку. То есть то, на сколько  нужно увеличить
полученное количство байт определяю как div 2-1.
Проблема в том, что мой результат при этом не совпадает
То есть после первого вызова or смещение дложно быть увеличено на 5,
у меня оно увеличилось на 6. После второго вызова должно быть увеличено на 6,
у меня увеличилось на 9 и т.д. Смещения для других команд у меня совпадают
так как я их считаю просто путем определения количества пробелов.}
r:=(length(cc) div 2-1); //case length(cc) of 2: r:=0; 4: r:=1; 8: r:=3; end;
t:=dollar(cop)+r; //writeln(t);
com_or:=cop; end;
//----------jb----------
function com_jb(k:string):string;
var x,y:string; a,b:integer;
begin k:=k+':';
...
if (a1='jb') and(jbprov=true) then m:=m+1;
if a1='or' then m:=m+t;
